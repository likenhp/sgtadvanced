...
class Student{
	...
	*/
	update(field, value){
		if(this.data.id === this.data[field] || 
			this.data.name === this.data[field] || 
			this.data.course === this.data[field] || 
			this.data.grade === this.data[field]){
			if(this.data[field] === this.data.grade){
				this.data[field] = parseInt(value);
				this.domElements.grade.text(this.data[field]);
				return true;
			}else if (this.data[field] === this.data.name){
	...
	*/
	render(){
		...
		this.deleteButton = $("<button>").text("delete").on("click", this.handleDelete);
		...
		var studentTableRow = this.domElements.row;
